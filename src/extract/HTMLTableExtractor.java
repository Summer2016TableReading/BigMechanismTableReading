	...
	private int getNumberOfColumns(Document doc){
		...
		if (colCount==0){
			...
			if (theads.size() > 0){
				...
				theads = doc.select("thead > tr");
				if (theads.size() > 0){
					theads = theads.last().select("td");
					colCount = theads.size();
				} else {
					theads = doc.select("tbody > tr");
					if (theads.size() > 0){
						theads = theads.last().select("td");
						colCount = theads.size();
					} 
					...
					if (colSize > 0){
						...
						if (theads.size() > 0){
							...
							while (headIt.hasNext()){
								t.addNextHeader(headIt.next().text());
										...
										if (td.hasAttr("colspan") && td.attr("colspan").matches("\\d+")){
											try{
												...
												if (Integer.parseInt(td.attr("colspan")) > 1){
													t.addCaption(td.text()); //TODO: split into groups? 
													continue;
												}
											}
											catch (NumberFormatException ne){
			...
			try {
				...
				for (int i = 0; i < tableRowElements.size(); i++) {
		            ...
		            for (int j = 0; j < rowItems.size(); j++) {
		            	...
		            	//
		               if(rowItems.get(j).attributes().hasKey("colspan") && rowItems.get(j).attributes().hasKey("rowspan")){
